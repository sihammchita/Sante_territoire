...
def load_all_data():
    ...
    pros_dept = pros_raw.groupby("dept").agg(
        ...
        nb_pros=(("specialite_libelle", lambda x: (x == "Médecin généraliste").sum()))+("specialite_libelle", lambda x: x.isin(SPECIALISTES).sum()),
...
with k4:
    st.markdown(f'<div class="kpi-card"><div class="kpi-value">{avg_acces:.1f} min</div><div class="kpi-label">Temps d\'accès moyen</div></div>', unsafe_allow_html=True)
with k5:
